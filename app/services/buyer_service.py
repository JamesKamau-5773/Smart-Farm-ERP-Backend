...
from app import db
from app.models.finance import Buyer, BuyerAuditLog, SalesLedger, PaymentStatus, Transaction, TransactionType, PaymentAllocation
...
class BuyerService:
    ...
    def allocate_payment(
        ...
        if previous_balance <= 0 and payment_amount > 0:
            # This is a payment on account (credit), which is allowed.
            pass
        ...
        for row in unpaid_rows:
            ...
            owed_on_sale = Decimal(row.total_cost) - Decimal(row.amount_paid)
            if owed_on_sale <= 0:
                continue
            ...
            row.amount_paid += apply_amount
            if row.amount_paid >= row.total_cost:
                row.payment_status = PaymentStatus.PAID
            else:
                row.payment_status = PaymentStatus.PARTIALLY_PAID
            ...
            allocations_made.append(
                {
                    ...
                    "remaining_on_entry": float(owed_on_sale - apply_amount),
