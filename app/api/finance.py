...
def _serialize_buyer(row):
    agreed_rate = float(row.agreed_rate_per_liter)
    return {
        'id': row.id,
        'name': row.name,
        'agreed_rate_per_liter': agreed_rate,
        'rate_per_liter': agreed_rate,
...
def list_ledger():
    ...
    serialized_rows = [_serialize_transaction(row) for row in rows]
    payload, meta = _paginate_items(serialized_rows)

    total_income = sum(float(row.amount) for row in rows if row.transaction_type == TransactionType.REVENUE)
    total_costs = sum(float(row.amount) for row in rows if row.transaction_type == TransactionType.EXPENSE)
...
def list_buyers():
    ...
    if tenant_id is None:
        return jsonify({'error': 'Missing or invalid tenant context.'}), 400
    rows = Buyer.query.filter_by(tenant_id=tenant_id).order_by(Buyer.name.asc()).all()
    q = (request.args.get('q') or '').strip().lower()
    if q:
        rows = [row for row in rows if q in (row.name or '').lower()]
    payload = [_serialize_buyer(row) for row in rows]
...
def create_buyer():
    ...
    agreed_rate = data.get('agreed_rate_per_liter', data.get('rate_per_liter'))
    if not name or agreed_rate is None:
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Buyer already exists for this tenant.'}), 409
...
def update_buyer(buyer_id):
    ...
    if 'agreed_rate_per_liter' in data or 'rate_per_liter' in data:
        buyer.agreed_rate_per_liter = data.get('agreed_rate_per_liter', data.get('rate_per_liter'))
